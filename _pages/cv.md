---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}


<style type="text/css"> body{ font-size: 12pt; text-align:justify; } </style> 

A passionate Computer Science Ph.D. researcher at [Tokyo Metropolitan University (TMU)](https://www.tmu.ac.jp/), Tokyo. Possessing a strong foundation gained through 3‑year research in Natural Language Processing, Machine Learning, and Deep Learning. My expertise extends to diverse projects encompassing Speech Recognition, Speaker Identification, Action Items Detection from Urdu text, Roman Urdu Sentiment Analysis, Social Bots Detection, and Botnet attacks in IoT and IoMT. I have published works in reputable journals and actively engage in collaborative research initiatives.




EDUCATION
------
* **PhD Computer Science**
  * **Tokyo Metropolitan University**  &nbsp;  &nbsp; _[ 04/04/2025 – Current ]_
  * **City:** Tokyo     &nbsp;  &nbsp;  **Country:** Japan
* **MSc Computer Science**
  * **University of Engineering and Technology Lahore**  &nbsp;  &nbsp; _[ 20/09/2021 – 17/11/2023 ]_
  * **City**: Lahore    &nbsp;  &nbsp;   **Country**: Pakistan 
    * **Research thesis titled**: ”A Technique To Enhance Social Media Reliance By Detection Social Bots”
    * **Courses**: Advanced Operating Systems, Advanced Computer Architecture, Theory of Computation, Advanced Algorithm Analysis, Research Methodologies, Web Engineering, Information Security Management
* **B.S in Computer Science**
  * **University of Sargodha**  &nbsp;  &nbsp; _[ 20/10/2017 – 27/08/2021 ]_
  * **City**: Sargodha    &nbsp;  &nbsp;  **Country**: Pakistan
    * **Final year project titled**: ”Planting the Seeds of Sustainability with Plantobook”
    * Plantobook, a React Native mobile application inspired by Instagram but designed specifically for plants.
    * **Courses**: Artificial Intelligence, Operation Research, Numerical Computing, Technical and Report Writing, Cloud Computing, Compiler Construction and more

PUBLICATIONS
------
  <ul>{% for post in site.publications reversed %}
    {% include archive-single-cv.html %}
  {% endfor %}</ul>


WORK EXPERIENCE
------
### 🎓 **Superior University Lahore** <br>
**City**: Lahore  &nbsp;  &nbsp;  **Country**: Pakistan <br>
**Computer science lecturer**      &nbsp; &nbsp; &nbsp; _[ 16/08/2023 – 17/02/2025 ]_ <br>
  * Instruct courses like Programming Fundamentals, Object‑Oriented Programming, Introduction To Logic’s, Data Structure and Algorithms.
  * Engage in a collaborative research eﬀort with Ph.D. Student on applying NLP techniques to classify toxic comments in Urdu. <br>
  
### 🔬 **Center for Language Engineering UET Lahore** <br>
**City**: Lahore   &nbsp;  &nbsp; **Country**: Pakistan <br>
**Research Oﬃcer**           &nbsp;  &nbsp; &nbsp;  _[ 15/08/2022 – 31/10/2023 ]_ <br>
  * Enriched a dataset of Urdu speakers and implemented innovative speaker identification models utilizing Wav2Vec, i‑vectors, and x‑vectors.
  * Contributed to the development of an automatic speech recognition system for Urdu language.
  * Performed research on action item detection from Urdu text.
  * Maintain Django web apps for automatic Meeting Minute generation in mixed Urdu‑English recorded meetings and developed Flask APIs to integrate the Machine Learning and Deep Learning model into the Meeting Minutes project.

### 💻 **CodeFreaks** <br>
**City**: Lahore &nbsp; &nbsp; **Country**: Pakistan <br>
**Junior React Native Developer** &nbsp; &nbsp; &nbsp; _[ 12/08/2021 – 05/08/2022 ]_ <br>
  * Implemented smooth animations and creative loading strategies to create a polished user experience.

CERTIFICATIONS
------
🐍 **Python Core**  &nbsp;  &nbsp;  _[ 08/2022 ]_  <br>
##### _**Sololearn**_ <br>

🌐 **WordPress**  &nbsp;  &nbsp;  _[ 08/2018 ]_  <br>
##### _**DigiSkills**_ <br>

💼 **Freelancing**  &nbsp;  &nbsp;  _[ 08/2018 ]_ <br>
##### _**DigiSkills**_ <br>


SKILLS
------
**Programming** <br>
  Python / Django / AI / Machine learning / Deep Learning / Natural Language Processing / PHP / WordPress / HTML / CSS / C / C++ / Redux  ECMAScript / SQL / CSS / React Native <br>

**Soft Skills** <br>
  Problem‑solving / Documentation / Time Management / Engaging Presentation / Teamwork <br>

**Miscellaneous** <br>
  Git / Microsoft Oﬃce / Kaldi toolkit / Firebase / Shell (Bash) / LATEX(Overleaf/R Markdown) / Ubuntu <br>

  
Talks
======
  <ul>{% for post in site.talks reversed %}
    {% include archive-single-talk-cv.html  %}
  {% endfor %}</ul>
  
Teaching
======
  <ul>{% for post in site.teaching reversed %}
    {% include archive-single-cv.html %}
  {% endfor %}</ul>
  
Service and leadership
======
* Currently signed in to 43 different slack teams
